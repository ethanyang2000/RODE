...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 shared_viewer=True, discrete_action=True, episode_limit=200):

        self.world = world
        self.world_length = self.world.world_length
        self.current_step = 0
        self.n_agents = self.world.policy_agents
        ...
        share_obs_dim = 0
        for i, agent in enumerate(self.n_agents):
            total_action_space = []
    ...
    def step(self, action_n):
        ...
        actions = np.eye(self.action_space[0].n)[action_n.cpu()]
        for i, agent in enumerate(self.n_agents):
        ...
        return np.sum(reward_n)/self.n, done_n[0], info_n[0]
    ...
    def get_obs_size(self):
        obs_size = np.array(self.observation_space[0].shape)
        ans = int(obs_size.prod()) 
        return ans
    ...
    def get_state(self):
        ...
        obs = np.concatenate(temp, axis=0)
        return obs
    
    def get_state_size(self):
        state_size = np.array(self.observation_space[0].shape)
        ans = int(state_size.prod()) * len(self.n_agents)
    ...
    def get_avail_actions(self):
        ...
        avail_actions = [[1]*total_actions for i in range(0, len(self.n_agents))]
    ...
    def _get_done(self, agent):
        if self.done_callback is None:
            if self.current_step >= self.world_length:
                ...
                return False
        return self.current_step >= self.done_callback
    ...
    def render(self, mode='human', close=False):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.shared_viewer:
                ...
                pos = self.n_agents[i].state.p_pos
